...
export default function ClientConversations() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Split Chat Grid */}
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3 min-h-[640px]">
        {/* Left: Conversations Thread List */}
        <Card className="lg:col-span-1 p-4 flex flex-col justify-between">
          <div>
            ...
            {isLoading ? (
              ...
              <ul className="max-h-[520px] space-y-1.5 overflow-y-auto pr-1">
                {conversations?.map((c) => (
                  <li
                    key={c.id}
                    className={`flex items-start gap-2.5 rounded-xl p-2.5 transition-all duration-200 cursor-pointer ${
                      selectedId === c.id
                        ? 'border border-brand-500/30 bg-brand-500/10 shadow-sm'
        ...
        <Card className="lg:col-span-2 p-6 flex flex-col justify-between">
          {!selectedId ? (
            <div className="flex h-full flex-col items-center justify-center py-20 text-center">
              <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-brand-500/10 text-brand-600 dark:text-brand-400 mb-3">
            ...
            <div className="flex h-full flex-col justify-between">
              ...
              {lead?.collectedInfo && Object.keys(lead.collectedInfo).length > 0 && (
                <div className="mb-4 rounded-xl border border-brand-500/20 bg-brand-500/5 p-3 text-xs dark:bg-white/[0.02]">
              ...
              <div className="flex-1 space-y-3.5 overflow-y-auto max-h-[420px] pr-2">
                {messages?.map((m) => (
