...
export default function AdminUsers() {
  ...
  const [activeFilter, setActiveFilter] = useState<'ALL' | 'SUPER_ADMIN' | 'ADMIN'>('ALL');
    ...
    <div className="space-y-8 animate-glass-entrance">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        ...
        <Button onClick={() => setShowCreate((s) => !s)} className="text-xs">
          {showCreate ? 'Close Staff Form' : '+ Add Administrator'}
        </Button>
      </div>
      ...
      {createdCredentials && (
        <div className="rounded-xl border border-emerald-500/30 bg-emerald-500/10 p-4 text-xs dark:bg-emerald-950/30 animate-tab-content">
          <p className="font-bold text-emerald-700 dark:text-emerald-300 mb-1">
            Account Successfully Configured for {createdCredentials.email}
          </p>
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <Tabs
          tabs={[
            { id: 'ALL', label: 'All Staff Accounts', count: allStaff.length },
            { id: 'SUPER_ADMIN', label: 'Super Administrators', count: superAdmins?.items?.length ?? 0 },
                ...
                {filteredStaff.length === 0 && (
                  <tr>
                    <td colSpan={6} className="py-12 text-center text-sm text-slate-400">
                      No staff accounts found matching the selected filter.
                    </td>
                  </tr>
                )}
              </tbody>
